...
team_names <- data.frame(nazwa=rep("a",30), skrot=rep('a',30), rok_zmian=rep(NA,30), poprzed_nazwa=rep(NA,30),
                         poprzed.skrot=rep('a',30))

team_names$nazwa <- c("Atlanta Hawks", "Boston Celtics", "Brooklyn Nets", "Charlotte Hornets", "Chicago Bulls",
...
team_names$skrot <- c("ATL", "BOS", "BRK", "CHO", "CHI", "CLE", "DAL", "DEN", "DET", "GSW", "HOU", "IND", "LAC",
                      "LAL", "MEM", "MIA", "MIL", "MIN", "NOP", "NYK", "OKC", "ORL", "PHI", "PHO", "POR", "SAC",
                      "SAS", "TOR", "UTA", "WAS")

team.names$rok_zmian <- c(NA, NA, 2013, 2015, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, 2014,
                          NA, 2009, NA, NA, NA, NA, NA, NA, NA, NA, NA)

team.names$poprzed_skrot <- c(NA, NA, "NJN", "CHA", NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA,
                              "NOH", NA, "SEA", NA, NA, NA, NA, NA, NA, NA, NA, NA)

team.names$poprzed_nazwa <- c(NA, NA, "New Jersey Nets", "Charlotte Bobcats", NA, NA, NA, NA, NA, NA, NA, NA, NA,
...
stats$Team<-sapply(stats$Team, function(x) team_names[team_names$nazwa==x,2])
...
team_story <- lapply(team_names$nazwa, function (x) x=data.frame()) # Tworzenie listy
names(team_story) <- team_names$nazwa # Przypianie nazw drużyn dla data.frameów

for ( i in 1:6) # Pętla ściagająca historie z 5 lat i wrzucająca to do team.story
{
    year <- paste0("201", i)
    
    for(j in 1:length(team_names$skrot)) # Pętla rozwiącująca problem zmian nazw drużyn.
    {
        team_names_tmp <- team_names$skrot[j]
        if((i==1 | i==2) & j==3)
        {
            team_names_tmp <- "NJN"
        }
...
Host <- unlist(sapply(games_story$Host, function(x) team_names[team_names$nazwa==x,2]))
names(Host)=NULL
games_story$Host=as.vector(Host)

Opponent_tmp=unlist(sapply(games_story$Opponent, function(x) team_names[team_names$nazwa==x, 2]))
